...
import time
def main(args):
    (train_file, test_file, output_file, num_tasks, rank) = args
    (X_train, y_train, id_train) = load_file(train_file)
    (X_test, y_test, id_test) = load_file(test_file, 2)
    dim = len(X_train[0])
    start = time.time()
...
if __name__ == '__main__':
    if len(sys.argv) != 6:
        print "Usage:./gpy_mtl2_sts.py <train-file> <test-file> <prediction-file> <num-tasks> <rank>"
        sys.exit(0)
    else:
        main(sys.argv[1:])
